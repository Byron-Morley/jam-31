...
public class FadingTextSystem extends IteratingSystem {
    ...
    protected void processEntity(Entity entity, float deltaTime) {
        ...
        Color startColor = colorInterp.start;
        Color currentColor = colorInterp.current;
        Color endColor = colorInterp.end;
        currentColor.set(startColor).lerp(endColor, colorInterp.interpolation.apply(colorProgress));

        Vector2 startPosition = positionInterp.start;
        Vector2 currentPosition = positionInterp.current;
        Vector2 endPosition = positionInterp.end;
